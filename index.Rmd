...
#Side-by-side Density plot of Political Ideology, by gender
p1 <- ggplotly(ggplot(data=subset(cox, Year==1980 & !is.na(Female) & !is.na(ideology7)), aes(x=ideology7, group=Female,fill = Female)) +
  theme(legend.position="none") +
  geom_density(adjust=1.5, alpha=.4,kernel = "gaussian",) +
  ggtitle("Political Ideology, by Gender | Left = 1980, Right = 2016") +
  scale_x_continuous(breaks = c(1,2,3,4,5,6,7), labels = c('Extr. Lib.', 'Lib.', 'Weak Lib', 'Moderate','Weak Cons.','Cons.','Extr. Cons')) +
  theme_ipsum())


p2 <- ggplotly(ggplot(data=subset(cox, Year==2016 & !is.na(Female) & !is.na(ideology7)), aes(x=ideology7, group=Female,fill = Female)) +
  theme(legend.position="none") +
  geom_density(adjust=1.5, alpha=.4,kernel = "gaussian",) +
  scale_x_continuous(breaks = c(1,2,3,4,5,6,7), labels = c('Extr. Lib.', 'Lib.', 'Weak Lib', 'Moderate','Weak Cons.','Cons.','Extr. Cons')) +
  theme_ipsum())
